        ...
        this.Icode.toUpperCase()==this.code.toUpperCase()?(()=>{
          ...
          login(apple).then(res=>{
            res.code?this.$message.error(res.msg):(()=>{
              ...
              res.data.isFirstLogin==1?this.$router.push({path:'revisePasswordHu',query:{userId:res.data.userId}})&& this.$cookies.set('oldPass',this.password):this.$router.push('/firstPage');
